    ...
    <>
      <header
        ...
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:h-20 lg:px-8">
          <Link href={brand.href} className="flex min-w-0 items-center gap-2.5 sm:gap-3" aria-label={`${brand.title} home`}>
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-extrabold leading-tight text-[#320056] sm:text-base">{brand.title}</p>
              <p className="hidden text-xs font-medium text-[#005768] sm:block">{brand.subtitle}</p>
            </div>
          </Link>
      ...
      <div className={`fixed inset-0 z-[60] lg:hidden ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}>
        ...
        <aside
          ...
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            ...
            <button type="button" onClick={() => setIsOpen(false)} className="rounded-xl p-2 text-slate-700 hover:bg-slate-100">
              <X className="h-5 w-5" />
            </button>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`rounded-2xl px-4 py-3 text-base font-medium ${
                    active ? "bg-[#320056] text-white" : "text-slate-700 hover:bg-slate-100"
                  }`}>
                  {page.label}
                </Link>
              );
