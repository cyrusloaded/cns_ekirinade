...
//   const [form, setForm] = useState<PortalSetting>({title: "Student Portal Coming Soon", description: "", launchAt: "", isEnabled: true});
//   const [status, setStatus] = useState("");

//   useEffect(() => {
//     async function load() {
//       const response = await fetch("/api/site/student-portal", {cache: "no-store"});
...
//       <header className="mb-8 flex flex-col gap-4 rounded-[2rem] bg-gradient-to-r from-[#320056] to-[#005768] px-6 py-8 text-white shadow-xl sm:px-8">
//         <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/15"><Timer size={28} /></div>
//         <div>
//           <h1 className="text-3xl font-black tracking-tight">Student Portal Timer</h1>
//           <p className="mt-2 max-w-2xl text-sm leading-7 text-white/75">Set the public student portal coming-soon title, message, and countdown time from the admin dashboard.</p>
//         </div>
//       </header>

//       <section className="max-w-3xl rounded-[1.75rem] bg-white p-6 shadow-sm ring-1 ring-slate-100 sm:p-8">
//         <div className="grid gap-5">
//           <label className="space-y-2 text-sm font-bold text-[#320056]">
//             <span>Coming Soon Title</span>
//             <input value={form.title} onChange={(event) => setForm((current) => ({...current, title: event.target.value}))} className="w-full rounded-2xl border-none bg-[#f2f4f7] px-4 py-4 text-sm outline-none focus:ring-2 focus:ring-[#320056]/20" />
//           </label>

//           <label className="space-y-2 text-sm font-bold text-[#320056]">
//             <span>Description</span>
//             <textarea value={form.description} onChange={(event) => setForm((current) => ({...current, description: event.target.value}))} className="min-h-32 w-full rounded-2xl border-none bg-[#f2f4f7] px-4 py-4 text-sm outline-none focus:ring-2 focus:ring-[#320056]/20" />
//           </label>

//           <label className="space-y-2 text-sm font-bold text-[#320056]">
//             <span>Launch Date & Time</span>
//             <input type="datetime-local" value={form.launchAt || ""} onChange={(event) => setForm((current) => ({...current, launchAt: event.target.value}))} className="w-full rounded-2xl border-none bg-[#f2f4f7] px-4 py-4 text-sm outline-none focus:ring-2 focus:ring-[#320056]/20" />
//           </label>

//           <label className="flex items-center justify-between rounded-2xl bg-[#f7f9fc] px-5 py-4 text-sm font-bold text-[#320056]">
//             <span>Show coming-soon page</span>
//             <input type="checkbox" checked={form.isEnabled} onChange={(event) => setForm((current) => ({...current, isEnabled: event.target.checked}))} className="h-5 w-5 rounded border-slate-300 text-[#320056]" />
//           </label>

//           {status && <p className="rounded-2xl bg-[#f7f9fc] px-4 py-3 text-sm font-semibold text-slate-600">{status}</p>}

//           <button type="button" onClick={() => void save()} className="inline-flex w-fit items-center gap-2 rounded-full bg-[#320056] px-7 py-3 text-sm font-black text-white shadow-lg shadow-[#320056]/20">
