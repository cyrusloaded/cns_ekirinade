...
// import {CalendarDays, ChevronLeft, ChevronRight, Eye, EyeOff, ImageIcon, PencilLine, Plus, RefreshCw, Save, Search, Trash2, X} from "lucide-react";
...
//   return new Intl.DateTimeFormat("en", {day: "2-digit", month: "short", year: "numeric"}).format(new Date(page.updatedAt));
// }
...
//   return name === "image" || name === "src" || name.includes("image") || name.includes("photo") || name.includes("portrait") || name.includes("avatar") || (typeof value === "string" && /\.(jpg|jpeg|png|webp|gif|svg)$/i.test(value));
// }
...
//   const value = item[key] ?? item.excerpt ?? item.desc ?? item.note ?? item.role;
//   return typeof value === "string" && value.trim() ? value : "No description yet.";
// }

// function updateByPath(item: ItemRecord, path: Path, value: unknown): ItemRecord {
...
//       if (isRecord(current) && typeof segment === "string") current[segment] = value;
//       return;
//     }
...
//     else if (isRecord(parent) && typeof segment === "string") parent = parent[segment];
//   }
...
//   if (isRecord(sample)) return Object.fromEntries(Object.keys(sample).map((key) => [key, ""]));
//   return "";
// }
...
//   async function saveCollection(nextItems: ItemRecord[], successMessage: string) {
//     if (!page) return;
//     const nextContent = setAtPath(page.content, collectionPath, nextItems);
//     const response = await fetch(`/api/cms/${page.slug}`, {
//       method: "PUT",
//       headers: {"Content-Type": "application/json"},
//       body: JSON.stringify({slug: page.slug, title: page.title, seoTitle: page.seoTitle ?? "", seoDescription: page.seoDescription ?? "", content: nextContent}),
//     });
//     if (!response.ok) {
...
//     await saveCollection(nextItems, `${getItemText(draft, titleKey)} saved and ${draft.published === false ? "kept as draft" : "published"}.`);
//     closeEditor();
//   }
...
//     await saveCollection(nextItems, `${getItemText(item, titleKey)} ${nextItems[index].published === false ? "unpublished" : "published"}.`);
//   }
...
//         nextDraft = updateByPath(nextDraft, altPath, imageAltFromFileName(file.name));
//       }
//       setDraft(nextDraft);
...
//     const inputClass = "w-full rounded-xl border-none bg-[#f2f4f7] px-4 py-3 text-sm text-slate-700 outline-none ring-1 ring-transparent transition focus:ring-[#320056]/25";

//     if (Array.isArray(value)) {
//       return (
//         <div className="rounded-2xl border border-slate-100 bg-white p-4 md:col-span-2">
//           <div className="mb-3 flex items-center justify-between gap-3">
//             <p className="text-xs font-black uppercase tracking-[0.18em] text-[#320056]">{titleCase(label)}</p>
//             <button type="button" onClick={() => setDraft((current) => updateByPath(current, path, [...value, emptyArrayItem(value)]))} className="rounded-full bg-[#320056] px-4 py-2 text-xs font-bold text-white">Add item</button>
//           </div>
//           <div className="space-y-3">
//             {value.map((child, index) => (
//               <div key={`${pathKey(path)}-${index}`} className="rounded-xl bg-[#f7f9fc] p-3">
//                 <div className="mb-2 flex justify-between">
//                   <span className="text-xs font-bold text-slate-400">Item {index + 1}</span>
//                   <button type="button" onClick={() => setDraft((current) => removeByPath(current, [...path, index]))} className="text-red-600"><Trash2 size={14} /></button>
//                 </div>
//                 {isRecord(child) ? renderObject([...path, index], child) : renderField([...path, index], index, child)}
//               </div>
//             ))}
//           </div>
//         </div>
...
//       return (
//         <label className="flex items-center justify-between rounded-xl bg-[#f7f9fc] px-4 py-3 text-sm font-bold text-[#320056]">
//           <span>{titleCase(label)}</span>
//           <input type="checkbox" checked={value} onChange={(event) => setDraft((current) => updateByPath(current, path, event.target.checked))} className="h-5 w-5 rounded border-slate-300 text-[#320056]" />
//         </label>
//       );
//     }
...
//       const key = pathKey(path);
//       return (
//         <div className="space-y-3 rounded-2xl bg-[#f7f9fc] p-4 text-sm font-bold text-[#320056]">
//           <div className="flex items-center justify-between">
//             <span className="text-[11px] uppercase tracking-[0.14em] text-slate-400">{titleCase(label)}</span>
//             <ImageIcon size={16} className="text-[#005768]" />
//           </div>
//           {imageValue ? <img src={imageValue} alt={String(draft.alt || title)} className="h-36 w-full rounded-xl object-cover" /> : <div className="flex h-36 items-center justify-center rounded-xl bg-white text-slate-300"><ImageIcon /></div>}
...
//             {uploadingField === key ? "Uploading image..." : "Choose image from device"}
//             <input type="file" accept="image/*" className="sr-only" disabled={uploadingField === key} onChange={(event) => void uploadImage(path, event.target.files?.[0] ?? null)} />
//           </label>
//           <input value={imageValue} onChange={(event) => setDraft((current) => updateByPath(current, path, event.target.value))} className={inputClass} placeholder="Saved image path" />
//         </div>
//       );
//     }

//     const valueText = value == null ? "" : String(value);
//     const isLong = valueText.length > 70 || /description|body|excerpt|statement|note|quote|objective/i.test(String(label));
//     return (
//       <label className="space-y-2 text-sm font-bold text-[#320056]">
//         <span className="text-[11px] uppercase tracking-[0.14em] text-slate-400">{titleCase(label)}</span>
//         {isLong ? (
//           <textarea value={valueText} onChange={(event) => setDraft((current) => updateByPath(current, path, event.target.value))} className={`${inputClass} min-h-28`} />
//         ) : (
//           <input value={valueText} onChange={(event) => setDraft((current) => updateByPath(current, path, event.target.value))} className={inputClass} />
//         )}
//       </label>
//     );
//   }
...
//           <div key={`${pathKey(path)}-${key}`} className={isRecord(value) || Array.isArray(value) ? "md:col-span-2" : ""}>{renderField([...path, key], key, value)}</div>
//         ))}
//       </div>
//     );
//   }
...
//           <div>
//             <h1 className="font-sans text-2xl font-extrabold tracking-tight text-[#320056]">{title}</h1>
//             <p className="mt-1 text-sm font-medium text-slate-500">{subtitle}</p>
//           </div>
//           <div className="relative">
//             <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={17} />
//             <input value={searchTerm} onChange={(event) => {setSearchTerm(event.target.value); setCurrentPage(1);}} className="w-full rounded-full border-none bg-[#e6e8eb] py-3 pl-10 pr-4 text-sm outline-none ring-1 ring-transparent transition focus:ring-[#320056]/20 sm:w-72" placeholder={searchPlaceholder} />
//           </div>
//         </div>
//       </header>
...
//                 <span className="inline-flex items-center gap-2 rounded-full bg-[#320056]/5 px-4 py-2 text-sm font-bold text-[#320056]"><span className="h-2 w-2 rounded-full bg-teal-500" /> {activeCount} Published {itemLabel}s</span>
//                 <span className="inline-flex items-center gap-2 rounded-full bg-[#320056]/5 px-4 py-2 text-sm font-bold text-[#320056]"><span className="h-2 w-2 rounded-full bg-amber-500" /> {pendingCount} Drafts</span>
//               </div>
//               <div className="flex gap-3">
//                 <button onClick={load} type="button" className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-5 py-3 text-sm font-bold text-slate-600 transition hover:bg-slate-200"><RefreshCw size={16} /> Refresh</button>
//                 <button onClick={openCreate} type="button" className="inline-flex items-center gap-2 rounded-full bg-gradient-to-br from-[#320056] to-[#4b0082] px-5 py-3 text-sm font-bold text-white shadow-lg shadow-[#320056]/20"><Plus size={16} /> Add New {itemLabel}</button>
//               </div>
//             </div>
//           </div>
//           <div className="flex items-center justify-center gap-4 rounded-[1.4rem] border border-[#005768]/10 bg-[#50d9fe]/10 p-5"><CalendarDays className="text-[#005768]" size={24} /><div><p className="text-[10px] font-black uppercase tracking-tight text-[#005768]">Last Update</p><p className="text-sm font-extrabold text-slate-800">{updatedDate(page)}</p></div></div>
//         </section>
...
//             return (
//               <article key={`${getItemText(item, titleKey)}-${items.indexOf(item)}`} className="group relative overflow-hidden rounded-xl border-l-4 border-[#005768] bg-white p-5 transition-all hover:shadow-xl hover:shadow-[#320056]/5">
//                 <div className="flex flex-col gap-6 md:flex-row md:items-center">
//                   <div className="flex h-28 w-full shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[#e6e8eb] text-[#320056] md:w-44">
//                     {typeof image === "string" && image ? <img src={image} alt={String(item.alt || getItemText(item, titleKey))} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /> : <ImageIcon size={38} />}
//                   </div>
//                   <div className="min-w-0 flex-1 space-y-2">
//                     <div className="flex flex-wrap items-center gap-2">
//                       {typeof primaryTag === "string" && primaryTag && <span className="rounded bg-[#f0dbff] px-2 py-1 text-[10px] font-black uppercase tracking-widest text-[#320056]">{primaryTag}</span>}
//                       {typeof secondaryTag === "string" && secondaryTag && <span className="rounded bg-[#b3ebff] px-2 py-1 text-[10px] font-black uppercase tracking-widest text-[#005768]">{secondaryTag}</span>}
//                       <span className={`rounded px-2 py-1 text-[10px] font-black uppercase tracking-widest ${item.published === false ? "bg-amber-100 text-amber-700" : "bg-teal-100 text-teal-700"}`}>{item.published === false ? "Draft" : "Published"}</span>
//                     </div>
//                     <h2 className="font-sans text-xl font-extrabold text-[#320056]">{getItemText(item, titleKey)}</h2>
//                     <p className="max-w-3xl text-sm leading-6 text-slate-500 line-clamp-2">{getItemDescription(item, descriptionKey)}</p>
//                   </div>
//                   <div className="flex gap-2 self-end md:self-center">
//                     <button type="button" onClick={() => void togglePublish(item)} className="rounded-full p-3 text-[#005768] transition hover:bg-[#005768]/5" title={item.published === false ? "Publish" : "Unpublish"}>{item.published === false ? <Eye size={18} /> : <EyeOff size={18} />}</button>
//                     <button type="button" onClick={() => openEdit(item)} className="rounded-full p-3 text-[#320056] transition hover:bg-[#320056]/5" title={`Edit ${itemLabel}`}><PencilLine size={18} /></button>
//                     <button type="button" onClick={() => void deleteItem(item)} className="rounded-full p-3 text-red-600 transition hover:bg-red-50" title={`Delete ${itemLabel}`}><Trash2 size={18} /></button>
//                   </div>
//                 </div>
//               </article>
//             );
//           }) : <div className="rounded-2xl bg-white p-8 text-sm font-semibold text-slate-500">{emptyText || `No ${itemLabel.toLowerCase()} records found.`}</div>}
//         </section>

//         <footer className="flex flex-col gap-4 border-t border-slate-100 py-6 sm:flex-row sm:items-center sm:justify-between">
//           <p className="text-sm font-semibold text-slate-400">Showing <span className="text-slate-800">{filteredItems.length ? (currentPage - 1) * PAGE_SIZE + 1 : 0}-{Math.min(currentPage * PAGE_SIZE, filteredItems.length)}</span> of <span className="text-slate-800">{filteredItems.length}</span> {itemLabel}s</p>
//           <div className="flex gap-2">
//             <button type="button" onClick={() => setCurrentPage((pageNumber) => Math.max(1, pageNumber - 1))} className="flex h-10 w-10 items-center justify-center rounded-full text-slate-400 transition hover:bg-slate-100"><ChevronLeft size={18} /></button>
//             {Array.from({length: totalPages}).map((_, index) => <button key={index} type="button" onClick={() => setCurrentPage(index + 1)} className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold transition ${currentPage === index + 1 ? "bg-[#320056] text-white shadow-md" : "text-slate-500 hover:bg-slate-100"}`}>{index + 1}</button>)}
//             <button type="button" onClick={() => setCurrentPage((pageNumber) => Math.min(totalPages, pageNumber + 1))} className="flex h-10 w-10 items-center justify-center rounded-full text-slate-400 transition hover:bg-slate-100"><ChevronRight size={18} /></button>
//           </div>
//         </footer>
//         <div className="rounded-2xl bg-white px-4 py-3 text-sm font-medium text-slate-600 shadow-sm">{status}</div>
//       </div>
...
//             <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-5 lg:px-8">
//               <div><h2 className="font-sans text-2xl font-extrabold text-[#320056]">{editingIndex === null ? `New ${itemLabel}` : `Edit ${itemLabel}`}</h2><p className="mt-1 text-sm text-slate-500">Create, update, publish, and save this content item to PostgreSQL.</p></div>
//               <button type="button" onClick={closeEditor} className="rounded-full p-2 text-slate-500 transition hover:bg-slate-100"><X size={20} /></button>
//             </div>
//             <div className="max-h-[calc(92vh-170px)] overflow-y-auto px-5 py-6 lg:px-8">{renderObject([], draft)}</div>
...
//               <p className="rounded-2xl bg-[#f7f9fc] px-4 py-3 text-sm font-medium text-slate-600">{status}</p>
//               <div className="flex justify-end gap-3"><button type="button" onClick={closeEditor} className="rounded-full px-6 py-3 text-sm font-bold text-slate-500 transition hover:bg-slate-100">Cancel</button><button type="button" onClick={() => void saveDraft()} className="inline-flex items-center gap-2 rounded-full bg-[#320056] px-7 py-3 text-sm font-bold text-white shadow-lg shadow-[#320056]/20"><Save size={16} /> Save {itemLabel}</button></div>
//             </div>
//           </div>
//         </div>
//       )}
