...
//   const activeCount = useMemo(() => (tab === "contacts" ? contacts.length : admissions.length), [tab, contacts.length, admissions.length]);

//   async function deleteEntry(id: string) {
//     const endpoint = tab === "contacts" ? `/api/contact-submissions/${id}` : `/api/admission-applications/${id}`;
...
//           <div>
//             <h2 className="text-xl font-bold text-[#320056]">Submissions manager</h2>
//             <p className="text-sm text-slate-500">Read and delete incoming contact messages and admission applications.</p>
//           </div>
//           <div className="rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-600">{activeCount} records shown</div>
//         </div>
...
//                 tab === key ? "bg-[#320056] text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
//               }`}
//             >
//               {label}
//             </button>
...
//                 <div>
//                   <h3 className="text-lg font-bold text-[#320056]">{item.fullName}</h3>
//                   <p className="mt-1 text-sm text-slate-500">{item.email} · {item.phoneNumber || "No phone"}</p>
//                   <p className="mt-1 text-sm text-slate-500">{new Date(item.createdAt).toLocaleString()}</p>
//                 </div>
//                 <button onClick={() => deleteEntry(item.id)} type="button" className="inline-flex items-center gap-2 rounded-2xl bg-red-50 px-4 py-3 text-sm font-bold text-red-600 transition hover:bg-red-100">
//                   <Trash2 size={16} /> Delete
//                 </button>
//               </div>
//               <p className="mt-3 inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-[#005768]">{item.inquiryType || "General"}</p>
//               <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-600">{item.message}</p>
//             </article>
//           ))}
//           {!contacts.length && <div className="rounded-[1.75rem] border border-dashed border-slate-300 bg-white p-6 text-sm text-slate-500">No contact submissions yet.</div>}
//         </div>
//       ) : (
//         <div className="grid gap-4">
//           {admissions.map((item) => (
...
//                   <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#005768]">Programme</p>
//                   <p className="mt-2 text-sm text-slate-600">{item.programmeOfInterest || "Not provided"}</p>
//                 </div>
//                 <div className="rounded-2xl bg-slate-50 p-4">
//                   <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#005768]">Previous qualifications</p>
//                   <p className="mt-2 text-sm text-slate-600">{item.previousQualifications || "Not provided"}</p>
//                 </div>
//               </div>
//               <div className="mt-4 rounded-2xl bg-slate-50 p-4">
//                 <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#005768]">Applicant statement</p>
//                 <p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-600">{item.statement || "No statement provided."}</p>
//               </div>
//             </article>
//           ))}
//           {!admissions.length && <div className="rounded-[1.75rem] border border-dashed border-slate-300 bg-white p-6 text-sm text-slate-500">No admission applications yet.</div>}
//         </div>
//       )}

//       <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">{status}</div>
//     </div>
//   );
// }
