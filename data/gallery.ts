import {
  BookOpen,
  GraduationCap,
  HeartPulse,
  LibraryBig,
  School,
  Stethoscope,
} from "lucide-react";

export const galleryFilters = [
  "All Moments",
  "Clinical Excellence",
  "Campus Life",
  "Graduation & Events",
];

export const galleryItems = [
  {
    id: 1,
    title: "Simulation Lab Practice",
    category: "Clinical Excellence",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAQTAUGHYCPd_svvCBPC8JtlfC72VJBU0kTwlsCrLty5arMXkDbjUn9EQp3NOxP0FcKszsS9RmxJvNB6kMrKZZnUrTc9j6V1qAWxUx5avgndjO5BUoer2EWyr3m55ihfOB41kAYJmgw40qaP3x1m9v2Jm4wFn73dCt1pEDd9AXGGOgHZd4oVRWC0RZN2Dl7WzbtzzpGy2ZhF-FMFEfOfKxC3wXBk_0A0WQNIYigpbi4KKuRn_ttwUB52WnJVZWSdl9R_8O6GfvAlUA",
    alt: "Nursing students in white lab coats practicing with high-fidelity medical mannequins in a modern hospital simulation ward",
    icon: Stethoscope,
    className: "md:col-span-8",
    imageClassName: "aspect-[4/3] sm:aspect-[16/9]",
    featured: true,
    hasAccent: true,
  },
  {
    id: 2,
    title: "Study Circles at the Commons",
    category: "Campus Life",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuC45sVmP5VNVaVQ47R0HMNUiqOgzxjZ9J2djREVhKusRyrHRUPQ1i2ZZc_JijNfPvuRDBvfpcrqtjyjLKA9u_TK9lckSsoOQOgZjwN-imZOMXk0kSynetCv8R1tedNaFea6KaPLy0OYYPdkg7QgOBSRxgrTVDT810YacI2Dj4en58EmD1hO-SxG8lALLd8pgLW3_ELC2nxWMg7TCnhnxf1M9oFdC6Xw_FxPm54mrNg5eGIURS4OoKk6-nK9PMdTPCQESFDs0q682YY",
    alt: "Diverse group of nursing students sitting on a lush green campus lawn studying and laughing together under a large shade tree",
    icon: School,
    className: "md:col-span-4",
    imageClassName: "aspect-[4/3] sm:aspect-square",
  },
  {
    id: 3,
    title: "Class of 2024 Pinning Ceremony",
    category: "Graduation & Events",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDWh2GqTajer2Y7EM6rTGfrXmWsIirGQKH0M2HqITQEy5GWmGu60rphmlbT2B8wJfPdpuiTtMZfH2n3ipNN9wxPgjmg9RGhEPd-63gBtsqIP8uC-Wdydru8e3e_fhGh63NSX4t_Splll20dvyf6soDI0dHDcvc732zb0MOQ3XVny4TRnrx0YeTbr-5BO52_d8Hyc-lCBuDCPQx26NW7eZcGku_SqpOqmxHLhYpMc8QeDsBhCPM3O38sWxd2fT0X7TA-QbgZLWHUPtg",
    alt: "A joyous nursing graduate in traditional academic gown holding a diploma and a small white cross, golden hour sunlight flare",
    icon: GraduationCap,
    className: "md:col-span-4 md:-mt-16",
    imageClassName: "aspect-[4/3] sm:aspect-[3/4]",
  },
  {
    id: 4,
    title: "The Art of Compassionate Care",
    category: "Nursing Excellence",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDKlGbuJlqrAIjbYkDysZ8SSPN5mwUWJ7MH-3mDnCY6lxc9dbQmW4aKnM-rrHktr380XN-gRfg3sTFcJSm4edXndiPUiRTOnHLLrEqUG4SSvOWMFPuwOp7SReilJXvBJ3u0rp-MoOfwU56Q7mCEgHENMH0ML82T3aWRoseCBqum72R5RhqD6BR43KRqgH38OevbjL0--db4nmj8-3PT9z4gHwdnTJ8r_r1WX4gRn0UbmDBaIER9UW8IJU5OwK8D9nhCjp2oYNL-IOE",
    alt: "Close up of a nurse hand gently holding an elderly patient's hand with warm soft lighting emphasizing human connection",
    icon: HeartPulse,
    className: "md:col-span-4",
    imageClassName: "aspect-[4/3] sm:aspect-video",
  },
  {
    id: 5,
    title: "Ekinrin-Adde Health Outreach",
    category: "Graduation & Events",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAYVWrNvDqyTu3KAYHUn8e4GSZkRmr2UPt3ObqIE5B3B2m-19oNuoriI_tqF9hxgKH7suypDoC9hILxlqNCmMnGfF68Hk2hLC7ol0B6qzKmLSnVdawvOHu-FwJP228qYj55gnfekz8W1CFLuMl0X4vqtQ-_hFvg8aFFxaSTb6JdsAr9vtcEx6a7KAsGv3kzxxvvhtpnKmw0bPlcZFSo713JZ1eV2UC8BmpkZOs6gZ_3OWlbE8fODFmZON2xskZJz_GqiTK9zZSUuCg",
    alt: "Nursing students providing free health screenings to community members in an outdoor village setting with bright natural light",
    icon: BookOpen,
    className: "md:col-span-4",
    imageClassName: "aspect-[4/3] sm:aspect-square",
    hasAccent: true,
  },
];

export const featuredLibrary = {
  title: "The Florence Nightingale Memorial Library",
  category: "Academic Facilities",
  description:
    "A sanctuary for research and academic excellence, housing the regions most comprehensive collection of medical literature.",
  image:
    "https://lh3.googleusercontent.com/aida-public/AB6AXuBpl21ldNRFyO_LK3KQOHmLnJ6CHgqvgMPpK6cJzoMz1qx6mrwR-demHHwJXuN9IQlVdHy19y-ciawe5me1y2ZB2dmIlLxSsI7ui2boEuimfakm-QhqVkgKJks35mwDJvUUxR36MBIpmrixknCQ20rvNKMuz9BKrgUwPbTqu_9c_VUKA4tmAEzc5d2a5kT5IDnuC5PoAwU_Rv_qehSc7MBEhYCsVKwvO569Vgdo5e-AqF4RMos3r3IY8DCdULTz8Hz8o0arBD0bXc0",
  alt: "Spacious modern college library with high ceilings, large windows, and students quietly studying at sleek wooden tables",
  icon: LibraryBig,
};
